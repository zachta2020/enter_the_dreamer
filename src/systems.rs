...
pub fn setup(mut commands: Commands, asset_server: Res<AssetServer>) {
    ...
    commands.spawn(LdtkWorldBundle {
        ldtk_handle: asset_server.load("Enter the Dreamer.ldtk"),
        ..default()
    });
}
...
pub fn movement(
    ...
    mut query: Query<(&mut Velocity, &mut MovementConfig, &GroundDetection), With<Player>>,
) {

    for (mut velocity, mut movement_config, ground_detection) in &mut query {
        let right = if input.pressed(KeyCode::Right) { 1. } else { 0. };
        let left = if input.pressed(KeyCode::Left) { 1. } else { 0. };

        if input.pressed(KeyCode::ShiftLeft) || input.pressed(KeyCode::ShiftRight) {
            velocity.linvel.x = (right - left) * movement_config.running_speed; 
        } else { 
            velocity.linvel.x = (right - left) * movement_config.walk_speed;
        }

        if input.just_pressed(KeyCode::Space) && (ground_detection.on_ground || movement_config.jump_count > 0) {
            movement_config.jump_count -= 1;
            velocity.linvel.y = movement_config.jump_speed;
        }

    }
}

/// Spawns Rapier2d collisions for the walls of a level
///
...
pub fn refresh_jumps(
    mut query: Query<(&mut MovementConfig, &Velocity, &GroundDetection), With<Player>>,
) {
    for (mut movement_config, velocity, ground_detection) in &mut query {
        if ground_detection.on_ground && velocity.linvel.y == 0.0 {
            movement_config.jump_count = movement_config.max_jump_count;
        }
    }
}
